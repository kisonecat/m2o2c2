...
if as $\mathbf{x}$ ``gets arbitrarily close to '' $\mathbf{p}$, the points $f(\mathbf{x})$ ``get arbitrarily close to $\mathbf{L}$''.  
	...
	\begin{definition}
		A function $f: \R^n \to \R^m$ is said to be \textit{continuous} at a point $\mathbf{p} \in \R^n$ if \(\displaystyle\lim_{\mathbf{x} \to \mathbf{p}} f(\mathbf{x}) = \mathbf{p}\)
	\end{definition}
...
\item show that it does have a limit by rewriting the expression algebraically as a continuous function, and just plug in to get the value of the limit.
	...
	\begin{question}
		...
		\begin{solution}
			\begin{hint}
				$\displaystyle\lim_{(x,y) \to (0,0)} \frac{x^2+xy}{x+y} = \lim_{(x,y) \to (0,0)} \frac{x(x+y)}{(x+y)}$
			\end{hint}
			\begin{hint}
				$\displaystyle\lim_{(x,y) \to (0,0)} \frac{x(x+y)}{(x+y)} = \lim_{(x,y) \to (0,0)} x = 0$
			\end{hint}
	...
	\begin{question}
		...
		\begin{solution}
			\begin{hint}
				$\displaystyle\lim_{(x,y) \to (3,3)} \frac{x^2-9}{xy-3y} = \lim_{(x,y) \to (3,3)} \frac{(x-3)(x+3)}{y(x-3)}$
			\end{hint}
			\begin{hint}
				$\displaystyle\lim_{(x,y) \to (3,3)} \frac{(x-3)(x+3)}{y(x-3)} = \lim_{(x,y) \to (3,3)} \frac{x+y}{y} = \frac{3+3}{3} = 2$
			\end{hint}
	...
	\begin{question}
		...
		\begin{solution}
			...
			\begin{hint}
				\begin{align*}
					\displaystyle\lim_{(x,y) \to (2,2)} \frac{x^2y-2y}{x-2} &= \lim_{(x,y) \to (2,2)} \frac{(x-2)(x+2)y}{x-2}\\
	...
	\begin{question}
		...
		\begin{solution}
			...
			\end{hint}
			\begin{hint}
				If we look at $\isplaystyle\lim_{(t,t) \to (0,0)} \frac{t}{t}$, this is just the limit of the constant $1$ function.  So the function approaches the limit $1$ along the 
				line $y=x$
			\end{hint}
			\begin{hint}
	...
	\end{hint}
	\begin{hint}
		So we have  $\displaystyle\lim_{x \to 0} \frac{1+k+x^2}{1-k} = \frac{1+k}{1-k}$ 
	\end{hint}
